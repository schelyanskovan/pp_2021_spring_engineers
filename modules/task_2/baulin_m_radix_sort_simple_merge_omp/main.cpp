...
TEST(RadixSort, works_4) {
    auto vect = std::vector<double>({1, 2, 3, 4});
    auto sorted = RadixSort(vect);
    ASSERT_EQ(sorted, std::vector<double>({1, 2, 3, 4}));
}
...
TEST(RadixSort, random_with_stdsotrcompare) {
  auto vect = GetRandomVector(100000);
  auto sorted = RadixSort(vect);
  std::sort(vect.begin(), vect.end());
  ASSERT_EQ(sorted, vect);
}
