...
TEST(JarvisTest, Simple_Check_No_Degenerates) {
  ...
  std::vector<Point> res2 = Jarvis::makeHullOmp(pts);
...
TEST(JarvisTest, Degenerate_Case_For_1_Point_CH_Is_This_One_Point) {
  ...
  std::vector<Point> res = Jarvis::makeHullOmp(pt);
...
TEST(JarvisTest, Big_Check_10000_Random_Points_By_Def) {
...
TEST(JarvisTest, Big_Check_10000_Random_Points_With_Seq) {
  std::mt19937 generator;
  generator.seed(322);
  ...
  std::cout << "OMP: " << t2 - t1 << std::endl << "SEQ: " << t3 - t2 << std::endl;
