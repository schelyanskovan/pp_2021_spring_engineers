...
void bitwise_sort_modOMP(BitsetIt first, BitsetIt last,
...
#pragma omp section
        {
            bitwise_sort_modOMP(first, le, bit - 1u, positive);
        }
#pragma omp section
        {
            bitwise_sort_modOMP(le, last, bit - 1u, positive);
        }
    }
}
...
#pragma omp section
        {
            bitwise_sort_modOMP(first, le, 62u, false);
        }
#pragma omp section
        {
            bitwise_sort_modOMP(le, last, 62u, true);
        }
    }
}
...
void radix_sortOMP(DoubleIt first, DoubleIt last) {
    ...
    bitwise_sortOMP(bits.begin(), bits.end());
...
#pragma omp parallel for
    ...
    radix_sortOMP(res.begin(), res.end());
    return res;
}
