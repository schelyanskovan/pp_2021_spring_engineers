...
SparseMatrix SparseMatrix::ParallelMult(const SparseMatrix& SM, int threads) {
  std::vector<double> res_values;
  std::vector<int> res_col_indexes(size + 1);
  std::vector<int> res_rows;
  SparseMatrix M_tr = this->Transpose();
...
#pragma omp parallel num_threads(threads)
  {
    ...
    local[thread_num].resize(3);
    local[thread_num][2].resize(size + 1, 0);
...
#pragma omp for
    for (int i = 0; i < size; ++i) {
      for (int j = 0; j < size; ++j) {
        ...
        if (tmp > eps || tmp < -eps) {
          local[thread_num][0].push_back(tmp);
          local[thread_num][1].push_back(j);
          ++local[thread_num][2][i + 1];
        }
      }
      local[thread_num][2][i + 1] += local[thread_num][2][i];
    }
  }

  for (int i = 0; i < threads; i++) {
    res_values.insert(res_values.end(), local[i][0].begin(), local[i][0].end());
    res_rows.insert(res_rows.end(), local[i][1].begin(), local[i][1].end());
    res_col_indexes.insert(res_col_indexes.end(), local[i][2].begin(),
                           local[i][2].end());
  }
