...
TEST(Matr_mupltiply, Correct_mupltiply_TBB_4x4) {
    std::vector<double> matr1
        = { 1.0, 2.0, 3.0, 4.0,
            4.0, 5.0, 6.0, 7.0,
            7.0, 7.0, 8.0, 9.0,
    ...
    std::vector<double> resultMatr
        = mupltiplyMatrixByMatrixTbb(matr1, matr2, 4, 4, 4);
    for (int i = 0; i < 16; i++) {
        ASSERT_EQ(expectedMatr[i], resultMatr[i]);
    }
}

TEST(Matr_mupltiply, Correct_mupltiply_TBB_16x16) {
...
TEST(Matr_mupltiply, Correct_mupltiply_TBB_36x36_4_threads) {
...
TEST(Matr_mupltiply, Correct_mupltiply_Tbb_36x36_9_threads) {
    const int matrRows = 36;
    std::vector<double> matr1 = generateMatrix(matrRows);
    std::vector<double> matr2 = generateMatrix(matrRows);
...
TEST(Matr_mupltiply, Correct_mupltiply_TBB_100x100_4_threads) {
