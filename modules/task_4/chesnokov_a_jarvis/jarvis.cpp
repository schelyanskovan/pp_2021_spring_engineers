...
std::vector<Point> Jarvis::makeHullStd(const std::list<Point>& in) {
  if (in.empty()) {
    // empty hull for empty list
    return std::vector<Point>();
  }
  ...
  for (int i = 0; i < THREAD_NUM; i++) {
    std::thread thr(Jarvis::makeHullStdSubRoutine, in, &hull, &turn, i, THREAD_NUM);
...
void Jarvis::makeHullStdSubRoutine(const std::list<Point>& in, std::vector<Point>* out,
