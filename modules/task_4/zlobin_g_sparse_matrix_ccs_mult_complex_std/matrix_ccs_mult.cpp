...
MatrixCCS STDMultiplication(const MatrixCCS& A, const MatrixCCS& B, int nThreads) {
    if (A._nCollumns != B._nRows) {
        throw "Can't multiply matrix which not corresponding";
    }
    ...
    int delta = B._nCollumns / nThreads;
    int reminder = B._nCollumns % nThreads;
    nThreads--;                                         // Comment for active wait thread
    std::thread* threads = new std::thread[nThreads];

    auto STDRunOn1Thread = [&rows, &values, &collumnsIndexes, &AT, &B](int begin, int end) {
        std::vector<int> tempRows(B._nRows);
    ...
    for (int m = 0; m < nThreads; m++) {
        int ibegin = m * delta;
        int iend = (m + 1) * delta;
        // iend += m == nThreads - 1 ? reminder : 0;    // Uncomment for active wait thread
        threads[m] = std::thread(STDRunOn1Thread, ibegin, iend);
    }
    STDRunOn1Thread(nThreads * delta, (nThreads + 1) * delta + reminder);   // Comment for active wait thread
