...
TEST(LinearFilter_STD, CalculateGaussFilterP_even_size) {
    ASSERT_ANY_THROW(CalculateGaussFilterSTD(4, 1.));
}

TEST(LinearFilter_STD, CalculateGaussFilterP_negative_sigma) {
    ASSERT_ANY_THROW(CalculateGaussFilterSTD(5, -1.));
}

TEST(LinearFilter_STD, CalculateGaussFilterP) {
    auto filter = CalculateGaussFilterSTD(3, 1.);
...
TEST(LinearFilter_STD, Filter_throw_with_incorrect_size) {
    std::vector<uint8_t> image(400, 0);
    std::vector<double> filter(9, 0);
    ASSERT_ANY_THROW(ApplyFilterSTD(filter, image, 40, 10, 4, 2));
}

TEST(LinearFilter_STD, Filter_doesnt_change_blank_image) {
    size_t size{10};
    std::vector<uint8_t> image(size * size, 255);
    auto result = ApplyFilterSTD(CalculateGaussFilterSTD(3, 1), image, size, size, 3, 2);
...
TEST(LinearFilter_STD, Filter_correct_values) {
    ...
    std::vector<uint8_t> correct_image{254, 254, 253, 254, 254, 254, 254, 254, 254, 253,
                                       ...
                                       255, 255, 255, 255, 255, 255, 255, 255, 255, 255};
    auto result = ApplyFilterSTD(CalculateGaussFilterSTD(3, 1), initial_image, 10, 10, 3, 2);
